...
function displayCard() {
    ...
    const vietnameseDiv = document.getElementById('vietnamese');
    ...
    vietnameseDiv.textContent = card.vietnamese;
    kanjiDiv.textContent = isFlipped ? `Kanji: ${card.kanji}` : 'Kanji: ???';
    hiraganaDiv.textContent = isFlipped ? `Hiragana: ${card.hiragana}` : 'Hiragana: ???';
    exampleDiv.textContent = isFlipped ? `Ví dụ: ${card.example}` : 'Ví dụ: ???';

    cardCountDiv.textContent = `Card ${currentCard + 1} of ${lessons[currentLesson].length}`;
}
